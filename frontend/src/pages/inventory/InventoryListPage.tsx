...
export function InventoryListPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Input
        placeholder="Search by product name, SKU, part number, or barcode..."
        value={q}
      ...
      <div className="overflow-x-auto">
        <div className="hidden min-w-full md:grid md:grid-cols-[1fr_160px_1fr_100px] md:gap-2 md:px-4 md:text-xs md:font-medium md:uppercase md:text-muted-foreground">
          <span>Product</span>
        ...
        <div className="mt-2 flex flex-col gap-2">
          {items?.map((item) => (
            <Link key={item.id} to={`/inventory/${item.id}`}>
              <Card>
                <CardContent className="flex items-center gap-3 p-4 md:grid md:grid-cols-[1fr_160px_1fr_100px] md:gap-2">
                  <div className="flex min-w-0 items-center gap-2 md:contents">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm text-muted-foreground md:hidden">
                        {item.product?.partNumber && `Part # ${item.product.partNumber} · `}
                        {item.location?.name}
                      </p>
                    </div>
                  </div>
